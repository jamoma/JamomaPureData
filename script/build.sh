...
if [ "x$ARCH" = "xrpi" ]; then
  ...
  /tmp/cmake/bin/cmake -DCROSS_COMPILER_PATH=`readlink -f ${HOME}/mingw-w64-install/` -DBUILD_JAMOMAPD=ON -DBUILD_JAMOMAMAX=OFF -DCMAKE_TOOLCHAIN_FILE=../JamomaPureData/Shared/CMake/toolchains/mingw-64.cmake -DJAMOMA_CORE_SRC_PATH=`readlink -f ${PWD}/../JamomaPureData/JamomaCore` -DPD_MAIN_PATH=`readlink -f $PWD/../pd` -DCMAKE_INSTALL_PREFIX="$PWD/JamomaInstall"  ../JamomaPureData/
