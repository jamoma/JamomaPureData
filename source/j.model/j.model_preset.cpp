...
//	defer(self, (method)model_preset_doread, msg, argc, argv);
	model_preset_doread(self,msg,argc,argv);
}
...
//				object_obex_dumpout(self, _sym_read, argc, argv);
				outlet_anything(x->dumpOut,gensym("read"), argc, argv);
			} else {
//				object_obex_dumpout(self, _sym_error, 0, NULL);
				outlet_anything(x->dumpOut, gensym("error"), 0 , NULL);
			}
		}
	}
}
...
//	defer(self, (method)model_preset_doread_again, NULL, 0, NULL);
	model_preset_doread_again(self);
}
...
//			object_obex_dumpout(self, _sym_read, 0, NULL);
			outlet_anything(x->dumpOut,gensym("read"),0,NULL);
		} else {
//			object_obex_dumpout(self, _sym_error, 0, NULL);
			outlet_anything(x->dumpOut,gensym("error"),0,NULL);
		}
...
//	defer(self, (method)model_preset_dowrite, msg, argc, argv);
	model_preset_dowrite(self,msg,argc,argv);
}
...
void model_preset_dowrite(TTPtr self, t_symbol *msg, long argc, t_atom *argv)
{
	...
	// stop filewatcher
	/* TODO write a filewatcher for PD
...
//				object_obex_dumpout(self, _sym_write, argc, argv);
				outlet_anything(x->dumpOut, gensym("write"),0,NULL);
			} else {
//				object_obex_dumpout(self, _sym_error, 0, NULL);
				outlet_anything(x->dumpOut, gensym("error"),0,NULL);
			}
	...
	// start filewatcher
	/* TODO rewrite a filewatcher for PD
	if (EXTRA->filewatcher)
...
//	defer(self, (method)model_preset_dowrite_again, NULL, 0, NULL);
	model_preset_dowrite_again(self);
}
...
//		defer_low(self, (method)model_preset_doread, gensym("read"), 1, &a);
		model_preset_doread(self, gensym("read"),1,&a);
...
//		defer_low((t_object*)x, (method)model_preset_dorecall, NULL, 1, &a);
		model_preset_dorecall((t_object*)x,NULL,1,&a);
		// replace filewatcher
		/* TODO rewrite a filewatcher for PD
		if (EXTRA->filewatcher) {
			filewatcher_stop(EXTRA->filewatcher);
...
void model_preset_edit(TTPtr self, t_symbol *msg, long argc, t_atom *argv)
{
	...
	*EXTRA->toEdit = *EXTRA->presetManager;
	...
	/* TODO rewrtite textEditor for Pd
	if (!EXTRA->textEditor) {
...
//		critical_exit(0);
		...
		outlet_anything(x->dumpOut, gensym("editor"),1,&a);
//        object_obex_dumpout(self, gensym("editor"), 1, &a);
