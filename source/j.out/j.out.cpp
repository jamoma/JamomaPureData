...
extern "C" void JAMOMA_EXPORT_MAXOBJ setup_j0x2eout(void)
{
...
#ifdef J_OUT_TILDE
    spec->_any = NULL;
    return (void)wrapTTModularClassAspdClass(kTTSym_OutputAudio, "j.out~", NULL, spec);
#endif
    ...
    return (void)wrapTTModularClassAsPdClass(kTTSym_Output, "j.out", NULL, spec);
#endif
#endif
}
...
void WrapTTOutputClass(WrappedClassPtr c)
{
    eclass_addmethod(c->pdClass, (method)out_assist,						"assist",				A_CANT, 0L);
	
#ifdef J_OUT_TILDE
    eclass_addmethod(c->pdClass, (method)out_dsp64,							"dsp64",				A_CANT, 0);
#endif
    
#ifdef J_OUT_MULTI
    eclass_addmethod(c->pdClass, (method)out_reset,                          "audio.reset",          A_CANT, 0);
    eclass_addmethod(c->pdClass, (method)out_setup,                          "audio.setup",          A_CANT, 0);
    eclass_addmethod(c->pdClass, (method)out_connect,                        "audio.connect",        A_OBJ, A_LONG, 0);
#endif
    ...
    // eclass_addmethod(c->pdClass, (method)out_subscribe,							"loadbang",				A_NULL, 0L);
#endif
#endif
		
    // no class_dspinit : it is done in wrapTTModularClassAspdClass for AUDIO_EXTERNAL
