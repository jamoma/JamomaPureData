...
// Definitions
void		WrapTTDataArrayClass(WrappedClassPtr c);
...
void		data_array_build(TTPtr self, t_symbol *msg, long argc, t_atom *argv);
...
extern "C" void JAMOMA_EXPORT_MAXOBJ setup_j0x2ereturn_array(void)
#elif defined(JMOD_MESSAGE)
extern "C" void JAMOMA_EXPORT_MAXOBJ setup_j0x2emessage_array(void)
#else
extern "C" void JAMOMA_EXPORT_MAXOBJ setup_j0x2eparameter_array(void)
#endif
{
	ModularSpec *spec = new ModularSpec;
    spec->_wrap = &WrapTTDataArrayClass;
    ...
    return (void)wrapTTModularClassAsPdClass(kTTSym_Data, "j.return_array", NULL, spec);
#else
    return (void)wrapTTModularClassAsPdClass(kTTSym_Data, "j.parameter_array", NULL, spec);
#endif
}
...
void WrapTTDataArrayClass(WrappedClassPtr c)
{
    ...
    eclass_addmethod(c->pdClass, (method)data_array,						"array",				A_GIMME, 0);
	
    eclass_addmethod(c->pdClass, (method)data_inc,							"+",					A_GIMME, 0);
...
void data_array_build(TTPtr self, t_symbol *msg, long argc, t_atom *argv)
{
    ...
    for (TTUInt32 i = lastSize + 1; i <= newSize; i++)
    {
        ...
        if (!jamoma_subscriber_create((t_eobj*)x, anObject, TTAddress(instanceAddress->s_name),  aSubscriber, returnedAddress, &returnedNode, &returnedContextNode))
        {
...
void data_array_return_value(const TTValue& baton, const TTValue& v)
{
    ...
    outlet_int((t_outlet*)x->outlets[index_out], i);
